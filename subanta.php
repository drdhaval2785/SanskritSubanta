...
elseif ( $_GET['cond1_1_1']==="6" || $_GET['cond2_1_2_1']==="6" || in_array($fo,array("idam")) )
{
    $sarvafinal = 1;
}
...
elseif ( ends(array($fo),array("asmad","yuzmad"),1)  ) // prAdhAnya and gauNatva pending.
{
    $sarvafinal = 1;
}
else
...
/* nakArAnta pulliGga Shat definition */ 
if ( $_GET['cond1_8']==="1")
{
    $samasa=1;
    $pradhana=0;
...
/* defining dhatus */
if (sub(array("supis","sutus","suhiMs","gir","pur"),blank(0),blank(0),0))
{
    $dhatu=1;
}
/* defining kvip */
if (sub(array("DvaMs"),blank(0),blank(0),0))
{
    $kvip=1;
    $dhatu=1;
}
...
if (sub(array("kruYc","krunc","daDfz","zRih"),array("+"),blank(0),0))
{
    $kvin=1;
}
if (sub(array("ftvij","daDfz","sfj","diS","zRih","aYcu","yuj","kruYc","anc","Anc","aYc","AYc","krunc"),array("+"),blank(0),0) && $kvin===1 )
{
...
{
    $text = one(array("yasm","asm","yuzm"),array("ivaya","vaya","yUya"),0);
...
if ($_GET["cond2_2_1"]==="1" || sub(array("catur"),array("+"),blank(0),0) )
{
    $tri="f";
}
else 
...
if ($tri === "f" && ends(array($fo),array("tri","catur"),1) && !ends(array($fo),array("stri"),1) && in_array($so,$sup))
{
...
if ($sarvanamasthana1 === 0 && in_array($so,$acsup) && $luk===0 )
{
...
if ( arr($text,'/['.flat($ac).'][n][+]/')  && !arr($text,'/['.flat($ac).'][n][+]$/') && !sub(array("Ahan"),blank(0),blank(0),0) && $sarvanamasthana1===1 && $sambuddhi===0 && $inhan===0 && $inhan1===0 && !in_array($fo,array("maGavan")) )
{
    $text = two($ac,array("n+"),$acdir,array("n+"),0);
...
if (sub(array("tisf","catasf"),array("+"),$acsup,0))
{
    ...
    echo "<p class = hn >This vArttika mandates a 'nuT' Agama. :</p>";
    echo "<p class = sa >नुमचिरतृज्वद्भावेभ्यो नुट्‌ पूर्वविप्रतिषेधेन (वा ४३७४) :</p>";
    echo "<p class = hn >यह वार्तिक नुट्‌ आगम का विधान करता है ।</p>";        $numacira=1;
    }
    else
    {
...
if ($bham === 1 && $vamanta===0 && arr($text,'/[a][n][+]/') && !(sub(array("udan+","Asan"),blank(0),blank(0),0) && !in_array($so,$yacibham)) ) 
{
...
/* ajAdyataSTAp (4.1.4) */
if ($gender==="f" && sub(array("a+"),$sup,blank(0),0) && in_array($so,$sup) && ($SaTsvasrAdi=0 || sub(array("a+"),$sup,blank(0),0)))
{
...
/* anApyakaH (7.2.112) */ print_r($text);
...
if ($kvip===1)
{
    $itpratyaya=array_merge($itpratyaya,array("k","p","i"));    
    $it=array_merge($it,array("k","p","i"));    
}
...
if ($sarvanamasthana1===1 &&  (( $dhatu===0 && itcheck(array("u","U","f","F","x","X"),1))  ||  ($ancu===1 && $kvin===1) ||  ( ($kvip===1 || $kvin===1)&& $dhatu===1 && arr(array($fo),'/[a][t]$/')) || $bhavat===1 ) )
{
...
/* acaH (6.4.138) */ 
if ( preg_match('/[aA][n][c]/',$fo) && $aniditAm === 1 && sub(array("ac","Ac"),array("+"),blank(0),0) && $bham===1 && $ancu===1)
{
...
// znu pending.  
if (($dhatu===1||$fo==="BrU") && arr($text,'/[iuIU][+]['.flat($ac).']/') && $pada==="pratyaya" && ($eranekaca===0 || ($eranekaca===1 && anekAca($fo)===false ) || (arr($text,'/[B][U][+]/')||$fo==="suDI") ) && $dfmBU===0 )
{
...
if (sub(array("a"),array("a","e","o"),blank(0),0) && !sub(array("pra","upa"),array("a","e","o"),blank(0),0) && $pada === "pratyaya" && $gogo===0)
{
    ...
    echo "<p class = sa >अतो गुणे (६.१.१७) :</p>";  $atogune = 1;
    display(0);
} else { $atogune = 0; } 
...
{
    $text = one(array("sras","Dvas","anaquh"),array("srad","Dvad","anaqud"),0);
...
if (arr($text,'/[z][+]/') && $pada==="pada" && sub(array("pipaWiz"),array("+"),blank(0),0) )
{
...
if ( ( sub(array("N"),$ku,array("+"),0) || sub(array("Y"),$cu,array("+"),0) || sub(array("R"),$Tu,array("+"),0) ||sub(array("m"),$pu,array("+"),0) ) && $ratsasya===0 && $pada==="pada" && $vriddhireci===0 ) // patch for nimittApAye naimittikasyApAyaH.
{
...
// mAMs+BhyAm -> mAn+bhyAm etc is pending 
...
$ekaco = array("gaD","gaB","gaQ","gAQ","gAD","gAQ","guD","guQ","gUQ","gfD","gfQ","graB","graQ","griQ","glaQ","qaQ","qiQ","quQ","daG","daG","daG","daG","diG","duG","duG","dfG","dfG","dfG","drAG","drAG","druG","druh","baD","baQ","bAQ","bAD","bAQ","bIB","buD","bfQ","beQ","braQ","druQ","gardaB");
$ekaco1 = array("GaD","GaB","GaQ","GAQ","GAD","GAQ","GuD","GuQ","GUQ","GfD","GfQ","GraB","GraQ","GriQ","GlaQ","QaQ","QiQ","QuQ","DaG","DaG","DaG","DaG","DiG","DuG","DuG","DfG","DfG","DfG","DrAG","DrAG","DruG","DruQ","BaD","BaQ","BAQ","BAD","BAQ","BIB","BuD","BfQ","BeQ","BraQ","DruQ","garDaB");
...
{
    echo "<p class = pa >stoH STunA STuH (8.4.41) is prevented by sthAnivadbhAva of allopa. The same result can be obtained by asiddhatva of bahiraGga allopa in kAryakAlapakSa.</p>";
