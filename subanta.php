...
if (in_array($so,array("wA","Nasi!","Nas")))
{
    $second = str_replace(array("wA","Nasi!","Nas"),array("ina","At","sya"),$second);
...
if (preg_match('/['.flat($ac).'][!]/',$so) && $pada=== "pratyaya" && $wa === 0)
{
...
/* cuTU (1.3.7) */
if (preg_match('/^[cCjJYwWqQR]/',$so) && $pada=== "pratyaya" && $wa ===0)
{
...
/* GeryaH (7.1.13) */
if (preg_match('/[a]$/',$fo) && $pada=== "pratyaya" && $so === "Ne")
{
...
if (preg_match('/^[lSkKgGN]/',$so) && $pada=== "pratyaya" && $taddhita === 0 && $wa === 0 & $Ne === 0)
{
    $second = substr($second,1);
...
if (preg_match('/[tTdDnsm]$/',$so) && $pada=== "pratyaya" && in_array($so,$sup) && $wa === 0)
{
...
if ((preg_match('/['.flat($hl).']$/',$so) && $pada=== "pratyaya" && !in_array($so,$sup)) || (in_array($so,$sup) && !preg_match('/[tTdDnsm]$/',$so)))
{
...
if ( $so === "Am" && (preg_match('/[a]$/',$fo)||$nadi===1||$Ap===1) && $start ===1)
{
...
if (in_array($so,array("Byas","sup")) && preg_match('/[a]$/',$fo) && $start ===1)
{
    $text = two(array("a"),array($second),array("e"),array($second),0);
...
/* nAmi (6.4.3) */
if ($second === "nAm" && preg_match('/['.flat($ac).']$/',$fo) && $start ===1)
{
...
if (sub($vrasca,prat('Jl'),blank(0),0) && $pada ==="pada")
{
    $text = one($vrasca,$vrashca,0);
    echo "<p class = sa >By vrazcabhrasjamRjayajarAjabhrAjacChazAM ca (8.2.35) :</p>";
    echo "<p class = sa >व्रश्चभ्रस्जमृजयजराजभ्राजशछां च (८.२.३५) :</p>";
    display(0);
}
if (sub($vrasca,blank(0),blank(0),0) && in_array($second,$vrasca))
{
    $text = one($vrasca,$vrashca,0);
    echo "<p class = sa >By vrazcabhrasjamRjayajarAjabhrAjacChazAM ca (8.2.35) :</p>";
    echo "<p class = sa >व्रश्चभ्रस्जमृजयजराजभ्राजशछां च (८.२.३५) :</p>";
...
if (preg_match('/['.flat($cu).']$/',$first) && $first === $input &&$pada === "pada")
{
...
{
    if ($r1 === 1 || $r2 ===1) {echo "<p class = sa >jhalAM jazo'nte is barred by sasajuSo ruH.</p>"; echo "<p class = sa >ससजुषो रुः से झलां जशोऽन्ते बाधित हुआ है । </p><hr>"; }
...
if (sub(array("Bo","Bago","aGo","a","A"),array("r@"),$ash,0)) 
{
    ...
    echo "<p class = sa >By bhobhagoaghoapUrvasya yo'zi (8.3.17) and vyorlaghuprayatnataraH zAkaTAyanasya (8.3.18) :</p>
       <p class = hn > In the opinion of zAkaTAyana, the padAnta yakAra and vakAra gets laghUccAraNa.</p>"; 
     echo "<p class = sa >भोभगोअघोअपूर्वस्य योऽशि (८.३.१७) तथा व्योर्लघुप्रयत्नः शाकटायनस्य (८.३.१८) :</p>
       <p class = hn > शाकटायन के मत में पदान्त वकार तथा यकार के लघूच्चारण होते हैं ।</p>";$bho = 1;
    display (0);
} else { $bho =0; }
...
if (sub($aa,$yv,blank(0),0) && (preg_match('/['.pc('ec').']$/',$first) || $bho === 1) && $pada === "pada")
{
echo "<p class = sa >By lopaH zAkalyasya (8.3.19) :</p>";
echo "<p class = sa >लोपः शाकल्यस्य (८.३.१९) :</p>";
...
$text = one(array("+"),array(""),0);
/* mo'nusvAraH (8.3.23) */ 
if (preg_match('/[m]$/',$first) && preg_match('/^['.pc('hl').']/',$second) && sub(array("m"),$hl,blank(0),0) && $pada ==="pada")
{
...
if (preg_match('/['.flat($hrasva).'][NRn]$/',$first) && preg_match('/^['.flat($ac).']/',$second) && $pada === "pada")
{
...
if((sub(array("i","I","u","U","k","K","g","G","N"),array("s"),array("u"),0) && preg_match('/^[s]/',$second) && $pada === "pratyaya") || (preg_match('/^[s]/',$second) && $pada === "pratyaya") )
{
$text = last("su","zu",0);
...
if ((preg_match('/['.flat($ac).']['.flat($hl).']$/',$second) || (preg_match('/['.flat($ac).']['.flat($hl).']$/',$first) && $input === $first )) && arr($text,'/['.flat($hl).']$/'))
{
    foreach ($text as $value)
    {
...
echo "<p class = sa >By jhalAसM jaz jhazi (8.4.53):</p>";
