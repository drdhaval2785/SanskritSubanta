...
echo "<p class = red >You entered: ".convert($fo)." + ".convert($so)." <a href = sandhi.html>Go Back</a></p>";
echo "</br>";
...
$nityabahuvacana = array("kati","tri","catur","paYcan","saptan","azwan","navan","daSan");
...
// 1 for obligatory. 2 for optional. 3 for no sarvanamasaJjJA.
...
elseif ( (ends(array($fo),$sarvanama,1) && !ends(array($fo),array("anyatama"),1) )|| (ends(array($fo),$sarvanamastri,1) && !ends(array($fo),array("anyatama"),1)) && ($samasa===0 || ($samasa===1 && $pradhana===1)))
{
    $sarvafinal = 1;
}
...
{
    $sarvafinal = 0;
    echo "<p class = hn >As anyatama is not enumerated in sarvAdi and anyatara is specifically enumerated, anyatama doesn't have sarvanAma saJjJA. </p>";
    echo "<p class = hn >सर्वादिगण में अन्यतर का गणन हुआ है और अन्यतम का नहीं है, इसलिए अन्यतम की सर्वनाम सञ्ज्ञा नहीं है ।</p>";        
    display(0);   
}
else
{
...
/* kRdatiG (3.1.93) */
if ($pada === "pratyaya" && !in_array($so,$tiG))
{
...
/* SNAntA Sat (1.1.24) */
if (arr($text,'/[zn][+]/') && ($sankhya===1 || ends(array($fo),array("paYcan","saptan","zwan","navan","daSan"),1)) && ($samasa===0 || $samasa===1 && $pradhana===1))
{
    $shat = 1;
...
elseif ( $gender==="f" && (( arr($text,'/[IU][+]/') && $nityastri===1) || in_array($fo,array("bahuSreyasI","aticamU"))))
{
...
if (sub(array("ftvij","daDfz","sfj","diS","zNih","aYcu","yuj","kruYc"),array("+"),blank(0),0) && $kvin===1 )
{
...
/* kimaH kaH (7.2.103) */
if (sub(array("kim","kaka"),array("+"),blank(0),0) && in_array($so,$sup))
{
...
if (arr($text,'/(jar)([aA])[+]/') && $pada=== "pratyaya"  && in_array($so,$acsup) )
    {
...
/* diva ut (6.1.131) */
if (sub(array("div+"),$hlsup,blank(0),0))
{
...
if (arr($text,'/[aI][+][SI]/') && $bham===1 && in_array($so,array("O","Ow")))
{
...
if (sub(array("in","han","pUzan","aryaman"),array("+"),array("su!"),0) && in_array($so,array("su!")) && $sambuddhi===0)
{
...
if (sub(array("in","han","pUzan","aryaman"),array("+"),$sarvanamasthana,0) && $sambuddhi===0 && $inhan===0)
{
...
if (arr($text,'/['.flat($ac).'][n][+]/') && !arr($text,'/['.flat($ac).'][n][+]$/') && $sarvanamasthana1===1 && $sambuddhi===0 && $inhan===0 && $inhan1===0  )
{
    $text = two($ac,array("n+"),$acdir,array("n+"),0);
...
if ( ($samp ===1 || $samp1===1 )&& sub(array("+i+","+u+"),$ac,blank(0),0) )
{   
...
if ( sub(array("h"),array("n"),blank(0),0) && arr(array($fo),'/[h][a][n]/'))
{
...
$tyadadinamah = array("dva+a","tya+a","ta+a","eta+a","ida+a","ada+a","eka+a","idaka+a");
$tyadadinamah1 = array("dva","tya","ta","eta","ida","ada","eka","idaka");
if (sub($tyadadi,array("+"),$sup,0) && in_array($fo,$tyadadi) && $idamoma===0)
{
    ...
    display(3);
    $text = two($tyadadinamah,$sup,$tyadadinamah1,$sup,0);
    echo "<p class = sa >By ato guNe (6.1.96) :</p>";
    echo "<p class = sa >अतो गुणे (६.१.९६) :</p>";
    display(0);
}
/* tadoH saH sAvanantyayoH (7.2.106) */
$tyadadinamah3 = array("dva","tya","eta","ta","ida","ada","eka","idaka");
$tyadadinamah2 = array("dva","sya","eza","sa","isa","asa","eka","isaka");
...
if (sub(array("ida+","eta+","idaka+"),blank(0),blank(0),0) && in_array($fo,array("idam","etad","idakam")) && $anvadesha===1 && in_array($so,array("am","Ow","Sas","wA","os")))
{
    $text = one(array("ida+","eta+","idaka+"),array("ena+","ena+","ena+"),0);
...
/* anApyakaH (7.2.112) */
if (sub(array("ida+"),$acsup,blank(0),0) && $fo==="idam" && in_array($so,$tRtIyAdiSvaci))
{
    $text = one(array("ida+"),array("ana+"),0);
...
/* hali lopaH (7.2.113) */
if (sub(array("ida+"),$hl,blank(0),0) && $fo==="idam" && !in_array($so,array("jas")))
{
    $text = one(array("ida+"),array("a+"),0);
...
/* dazca (7.2.109) */
if (sub(array("ida","idaka"),array("+"),blank(0),0) && in_array($fo,array("idam","idakam")) && in_array($so,$sup))
{
    $text = one(array("ida+","idaka+"),array("ima+","imaka+"),0);
...
$sarvanamastri1 = array("sarva","viSva","uBa","uBaya","atara","atama","anya","anyatara","itara","tva","nema","sima","pUrva","para","avara","dakziRa","uttara","apara","aDara","sva","antara","eka","dva",);
if ($Ap===1 && $sarvafinal !==0 && in_array($so,array("Ne","Nas","Ni")) && ends(array($fo),$sarvanamastri,1))
{ 
    if ( $sarvafinal === 2)
    {
    $text = last(array("Ne","Nas","Ni"),array("syA+Ne","syA+Nas","syA+Ni"),1);        
    $text = one(array("tIyA+syA+Ne","tIyA+syA+Nas","tIyA+syA+Ni"),array("tIya+syA+Ne","tIya+syA+Nas","tIya+syA+Ni"),0);            
    }
    else
    {
    $text = last(array("Ne","Nas","Ni"),array("syA+Ne","syA+Nas","syA+Ni"),0);        
    $text = one(array("tIyA+syA+Ne","tIyA+syA+Nas","tIyA+syA+Ni"),array("tIya+syA+Ne","tIya+syA+Nas","tIya+syA+Ni"),0);            
    }
...
/* yADApaH (7.3.113) */
if ($Ap===1 && in_array($so,array("Ne","Nas","Ni")))
{
...
if ($Ap===1 && in_array($so,array("Nasi!")) )
{
...
/* jasaH zI (7.1.17) */
if (arr($text,'/[a][+]/') && $pada=== "pratyaya" && $so === "jas" && in_array($fo,$sarvanama) && $nojas===0)
    {
...
if (arr($text,'/['.flat($ac).'][!]/')  )
{
    $itfinder = preg_split('/(['.flat($ac).'][!])/',$input,null,PREG_SPLIT_DELIM_CAPTURE);
    $itfinder[1] = str_replace("!","",$itfinder[1]); 
    $it=array_merge($it,array($itfinder[1]));
    $itfinder=array();
...
{
    if(substr($first,0,2) === "Yi") { $itpratipadika = array_merge($itpratipadika,array("Yi")); }
    if(substr($first,0,2) === "wu") { $itpratipadika = array_merge($itpratipadika,array("wu")); }
    if(substr($first,0,2) === "qu") { $itpratipadika = array_merge($itpratipadika,array("qu")); }
...
{
    $itfinder = preg_split('/([+][cCjJYwWqQR])/',$input,null,PREG_SPLIT_DELIM_CAPTURE);
    $itfinder[1] = str_replace("+","",$itfinder[1]);
    $itpratyaya=array_merge($itpratyaya,array($itfinder[1]));
    $itfinder=array();
...
{
    $itfinder = preg_split('/([+][z])/',$input,null,PREG_SPLIT_DELIM_CAPTURE);
    $itfinder[1] = str_replace("+","",$itfinder[1]);
    $itpratyaya=array_merge($itpratyaya,array($itfinder[1]));
    $itfinder=array();
...
/* GeryaH (7.1.13) */
if (arr($text,'/[a][+][N][e]/') && $pada=== "pratyaya" && $so === "Ne" && $sarva === 0)
{
...
{
    $itfinder = preg_split('/([+][lSkKgGN])/',$input,null,PREG_SPLIT_DELIM_CAPTURE);
    $itfinder[1] = str_replace("+","",$itfinder[1]);
    $itpratyaya=array_merge($itpratyaya,array($itfinder[1]));
    $itfinder=array();
...
{
    $itfinder = preg_split('/(['.flat($hl).']$)/',$input,null,PREG_SPLIT_DELIM_CAPTURE);
    $itpratyaya=array_merge($itpratyaya,array($itfinder[1]));
    $itfinder=array();
...
$it = array_merge($it,$itpratipadika,$itpratyaya); 
...
if ( !itcheck(array("i"),1) && arr($text,'/[n]['.pc('hl').'][+]/') && ( itcheck(array("k","G"),2)|| $ancu===1 ) && $kruJca===0)
{
    if ($nAJce===1)
    {
    $text = three(array("n"),$hl,array("+"),array(""),$hl,array("+"),1);        
    }
    else
    {
    $text = three(array("n"),$hl,array("+"),array(""),$hl,array("+"),0);        
    }
...
if (preg_match('/[t][i][r][a][s][a][n][c][u]/',$fo) && $aniditAm === 1 && sub(array("ac","Ac"),array("+"),blank(0),0) && $bham===0 && $ancu===1)
{
    $text = two(array("tirasac"),array("+"),array("tiriac"),array("+"),0);
...
if ($sarvanamasthana1===1 &&  (( $dhatu===0 && itcheck(array("u","U","f","F","x","X"),1) ) ||  ($ancu===1 && $dhatu===1) ) && $kruJca===0)
{
    $text = mit("n",0);
...
if ( $so === "Am" && $numacira===0 && (arr($text,'/[aiufx][+]/')) && !in_array($fo,$sarvanama) && $sut===0)
{
...
if ( $gender==="n" && $sarvanamasthana1===1 && arr($text,'/['.pc('ac').'][+]/') && $amipUrva === 0)
{
    $text = three($ac,array(""),array("+"),$ac,array("n"),array("+"),0);
    echo "<p class = sa >By napuMsakasya jhalacaH (7.1.72) :</p>";
    echo "<p class = sa >नपुंसकस्य झलचः (७.१.७२) :</p>";
    display(3);
}
if ( $gender==="n" && $sarvanamasthana1===1 && arr($text,'/['.pc('Jl').'][+]/') && $amipUrva === 0)
{
    $text = three(array(""),prat("Jl"),array("+"),array("n"),prat("Jl"),array("+"),0);
    echo "<p class = sa >By napuMsakasya jhalacaH (7.1.72) :</p>";
    echo "<p class = sa >नपुंसकस्य झलचः (७.१.७२) :</p>";
    display(3);
}
/* sAntamahataH saMyogasya (6.4.10) */
if (arr($text,'/['.flat($ac).'][n][s][+]/') && $sarvanamasthana1===1 && $sambuddhi===0)
{
...
if (arr($text,'/['.flat($ac).'][n][s][+]/') && ends(array($fo),array("mahat"),1) && $sarvanamasthana1===1 && $sambuddhi===0)
{
    $text = two($ac,array("ns+"),$acdir,array("ns+"),0);
    echo "<p class = sa >By sAntamahataH saMyogasya (6.4.10) :</p>";
    echo "<p class = sa >सान्तमहतः संयोगस्य (६.४.१०) :</p>";
...
// znu pending. 
if (($dhatu===1||$fo==="BrU") && arr($text,'/[iuIU][+]['.flat($ac).']/') && $pada==="pratyaya" && ($eranekaca===0 || ($eranekaca===1 && anekAca($fo)===false )))
{
...
if ($dhatu===1 && arr($text,'/['.flat($ac).']['.flat($hl).'][iI][+]['.flat($ac).']/') && $pada==="pratyaya" && anekAca($fo) && $eranekaca===1 && $nabhusu===0)
{
...
/* ato bhisa ais (7.1.9) */
if (arr($text,'/[a][+]/') && $so === "Bis" && $nedamadas===0 )
{
...
{
    $text = one($vasu,array("srad","Dvad","anaqud"),0);
...
if ((arr($text,'/[s][+]/') && $start===1  && $pada ==="pada" && $eg !== 1) )
{
    $text = one(array("s+"),array("r@+"),0);
    echo " <p class = sa >By sasajuSo ruH (8.2.66) :</p>";
    echo " <p class = sa >ससजुषो रुः (८.२.६६) :</p>";
       display(0);
}
if (preg_match('/[^'.pc('hl').'][s]$/',$first) && $start===1  && $pada ==="pada" && $eg !==1 )
{
...
if ((arr($text,'/[^'.pc('hl').'][s]$/')||arr($text,'/[^'.pc('hl').'][s][+]$/') ) && $start===1 && $eg !==1 )
{
//    $text = last(array("s\+"),array(""),0);
...
if (sub(array("ahan"),blank(0),blank(0),0) && !sub($noahan,blank(0),blank(0),0) && $first === "ahan" && !(in_array($so,$sup)) && $pada==="pada")
{ 
...
if (sub(array("ahan"),blank(0),blank(0),0) && !sub($noahan,blank(0),blank(0),0) &&(in_array($so,$sup)) && $pada === "pada")
{
    $text = one(array("ahan"),array("ahar@"),0);
...
if (arr($text,'/[n][+]$/')  && ( in_array($so,array("Ni")) || (in_array($so,array("su!")) && $sambuddhi===1)) && $bham===0 && $shi===0 && $ikoci===0 && $samyoga===0)
{
...
if (sub(array("maGavan"),blank(0),blank(0),0)  && !arr($text,'/['.flat($ac).'][n][+]$/') && arr($text,'/[v][a][n][+]$/') && in_array($so,$sup) && $bham===0 && $shi===0 && $ikoci===0 && $Gisambu===0 && $sambuddhi===0)
{
...
elseif (arr($text,'/[n][+]$/') && in_array($so,$sup) && $bham===0 && $shi===0 && $ikoci===0 && $samyoga===0 && $Gisambu===0 && $samyoga1===0)
{
    $text = two(array("n"),array("+"),array(""),array("+"),0);
    echo "<p class = sa >By na lopaH prAtipadikAntasya (8.2.7) :</p>";
    echo "<p class = sa >न लोपः प्रातिपदिकान्तस्य (८.२.७) :</p>";
...
if (arr($text,'/[n][+]['.pc('hl').']/') && ( $astana===1 || $Satcatur===1) )
{
    $text = two(array("n"),array("+"),array(""),array("+"),0);
    echo "<p class = sa >By na lopaH prAtipadikAntasya (8.2.7) :</p>";
    echo "<p class = sa >न लोपः प्रातिपदिकान्तस्य (८.२.७) :</p>";
...
elseif (arr($text,'/[n][+]['.pc('hl').']/')  && !arr($text,'/[n][+][e]/') && ($pada==="pada"|| $so==="Am" ) && in_array($so,$sup) && $bham===0 && $shi===0 && $ikoci===0 && $samyoga===0 && $Gisambu===0 && $samyoga1===0)
{
    $text = two(array("n"),array("+"),array(""),array("+"),0);
    echo "<p class = sa >By na lopaH prAtipadikAntasya (8.2.7) :</p>";
    echo "<p class = sa >न लोपः प्रातिपदिकान्तस्य (८.२.७) :</p>";
...
if (sub($hl,array("+"),blank(0),0) && $kvin===1 && $pada==="pada")
{   
...
if ( sub(array("M"),$hl,array("+"),0) && $bham===0 && $pada==="pada" && $vriddhireci===0)
{
    ...
    {
    $text = two(array("M"),array("+"),array("n"),array("+"),0);
...
if (arr($text,'/['.pc('Jl').'][+]/') && $pada === "pada" && $atobhis===0 )
{     
...
{
$text = two($iN1,array("+su","+sAm",),$iN1,array("+zu","+zAm"),0);
...
if(arr($text,'/([fF])([aAiIuUfFxXeoEOhyvrkKgGNpPbBmM+]*)([R])/') && $honante===0)
{
